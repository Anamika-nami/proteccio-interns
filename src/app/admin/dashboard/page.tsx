        ...
        <div className="flex items-center gap-2 flex-wrap">
          {[
            ...
            <button key={path} onClick={() => router.push(path)}
              className="text-xs text-gray-400 hover:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 rounded px-2 py-1 transition-colors">
              {label}
            </button>
          ))}
          <button onClick={async () => { const s = createClient(); await s.auth.signOut(); router.push('/admin/login') }}
            className="text-xs text-red-400 hover:text-red-300 focus:outline-none focus:ring-2 focus:ring-red-500 rounded px-2 py-1 transition-colors">
            Logout
          </button>
        </div>
      </nav>
                ...
                {interns.map(intern => (
                  <div key={intern.id} className="bg-gray-900 border border-gray-800 rounded-xl px-5 py-3.5 flex items-center justify-between hover:border-gray-600 transition-colors flex-wrap gap-3">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <button onClick={() => handleApprove(intern.id)} className="text-green-400 text-xs border border-green-800 px-2 py-1 rounded hover:border-green-600 transition-colors focus:outline-none focus:ring-2 focus:ring-green-500">Approve</button>
                      <button onClick={() => handleReject(intern.id)} className="text-yellow-400 text-xs border border-yellow-800 px-2 py-1 rounded hover:border-yellow-600 transition-colors focus:outline-none focus:ring-2 focus:ring-yellow-500">Reject</button>
                      <button onClick={() => handleDelete(intern.id)} className="text-red-400 text-xs border border-red-800 px-2 py-1 rounded hover:border-red-600 transition-colors focus:outline-none focus:ring-2 focus:ring-red-500">Delete</button>
                    </div>
                  </div>
                ))}
              </div>
